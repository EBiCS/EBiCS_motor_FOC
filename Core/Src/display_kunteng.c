...
void display_update(MotorState_t *MS_U) {
  ...
  ui16_wheel_period_ms=(uint16_t)(1000.0*(float)GEAR_RATIO/(float)ui32_erps_filtered);
	...
	ui8_tx_buffer[8] = (uint8_t) (MS_U->Battery_Current * MS_U->Voltage
			* CAL_BAT_V / 82010000); //Kalibrierung nach Binatone, empririsch ermittelt. Strom und Spannung in Milli, 13W pro digit
...
void check_message(MotorState_t *MS_D) {
	...
	for (ui8_j = 0; ui8_j <= 12; ui8_j++) {
		...
		if (ui8_j == 5)
			continue; // don't xor B5 (B7 in our case)
	...
	if (((ui8_crc ^ 10) == ui8_rx_buffer[5]) || // some versions of CRC LCD5 (??)
		...
		lcd_configuration_variables.ui8_wheel_size = ((ui8_rx_buffer[4] & 192)
				>> 6) | ((ui8_rx_buffer[2] & 7) << 2);
		lcd_configuration_variables.ui8_max_speed = (10
				+ ((ui8_rx_buffer[2] & 248) >> 3)) | (ui8_rx_buffer[4] & 32);
		lcd_configuration_variables.ui8_power_assist_control_mode =
				ui8_rx_buffer[4] & 8;
		lcd_configuration_variables.ui8_controller_max_current =
				(ui8_rx_buffer[7] & 15);
